...
async def mutations_inventory(interaction: discord.Interaction, user: discord.Member = None):
    user = user or interaction.user
    user_id = user.id
    raw = await get_mutated(user_id)
    inventory = await decrypt_inventory(raw)
    embed = discord.Embed(title=f"{user.name}'s Mutations Inventory", color=discord.Color.green())
    if inventory:
        temp = ""
